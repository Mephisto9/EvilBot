...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=557788167446 :+55 77 88168446\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo  C ',
					admin: '[❗] Este comando so pode ser usado pelos admininastros do grupo seu fdp',
					Badmin: '[❗] Este comando so pode ser usado quando o bot se torna um admininastro do grupo',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá como!\nVocê não está registrado no banco de dados, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Alucard|18`,
				}
			}
			...
			const ownerNumber = ["557788167446@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '557788167446@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse cara acaba de perder o ademir @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
	...
	case 'kbbi':
		...
		if (args.length < 1) return reply ('*PARCEIROS*: \nEaxy: https://youtube.com/c/EAXYFF\nYamii: https://youtube.com/c/TioYamiiroivemsmpaki\nSaad: https://youtube.com/channel/UCtX3fzhC-teLUpaS7Xz8nKA\nKazuma: https://youtube.com/channel/UChRgBlar0IKGAsseuBeNRgA')
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ALUCARD BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ALUCARD BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
	    ...
	    case 'kapankah':
            ...
            break
				...
				case 'simi':
					...
					teks = body.slice(5)
					anu = await simih(teks) //fetchJson(`https://mhankbarbars.herokuapp.com/api/samisami?text}`, {method: 'get'})
